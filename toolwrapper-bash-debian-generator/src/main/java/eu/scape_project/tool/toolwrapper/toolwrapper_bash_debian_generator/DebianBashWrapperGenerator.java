...
public class DebianBashWrapperGenerator extends ToolWrapperCommandline implements ToolWrapperGenerator {
  ...
  private static final List<String> DEBIAN_FILES = Arrays.asList("source/format", "compat", "control", "copyright",
    "dirs", "install", "MAN.manpages", "MAN.pod", "README", "rules");
  ...
  public DebianBashWrapperGenerator() {
    ...
    opt = new Option("a", "aggregate", false, "aggregates all artifacts in a single Debian package (use it with -d)");
   ...
   * @param operation
   *          if the artifact(s) is related to a particular {@link Operation}
   * @param outputDirectory
   *          directory where to place the created artifact(s)
   * 
   * */
  @Override
  public boolean generateWrapper(Tool tool, Operation operation, File outputDirectory) {
    this.tool = tool;
    this.operation = operation;
    ...
    if (verifyNeededDirectories(outputDirectory) && verifyToolspecSemanticAspects()) {
  ...
  private void initVelocity() {
    ...
    p.setProperty("runtime.log.logsystem.log4j.logger", "org.apache.velocity");
    Velocity.init(p);
  }
  ...
  private boolean verifyNeededDirectories(File outputDirectory) {
    ...
    if (!outputDirectory.exists() && !outputDirectory.mkdir()) {
      log.error("The directory \"" + outputDirectory.getAbsolutePath() + "\" cannot be created...");
      res = false;
    } else {
      // create a directory where the Debian package will end up
      File childDirectory = new File(outputDirectory, Constants.DEBIANBASHGENERATOR_DEBS_OUTDIRNAME);
      boolean operationResult = (childDirectory.exists() || childDirectory.mkdir());
      res = res && operationResult;
      if (!operationResult) {
        log.error("The directory \"" + childDirectory + "\" either not exists or cannot be created...");
      }
    }
    return res;
  }
  ...
  private boolean verifyToolspecSemanticAspects() {
    ...
    for (Operation op : tool.getOperations().getOperation()) {
      if (op.getDescription().length() > 60) {
        log.error("Description size for operation \"" + op.getName()
  ...
  private void addInputUsageInformationToContext(VelocityContext wrapperContext) {
    if (operation.getInputs().getStdin() != null) {
      wrapperContext.put("usageInputParameter", "-i STDIN");
      wrapperContext.put("usageInputParameterDescription", "-i STDIN > Read input from the STDIN");
    } else {
      ...
      for (Input input : operation.getInputs().getInput()) {
        String value = "-i " + input.getName() + (i == 0 ? "|STDIN" : "");
        ...
        String inputDescription = value + " > " + input.getDescription()
      ...
      wrapperContext.put("usageInputParameterDescription", uipd.toString());
      wrapperContext.put("usageInputParameterDescriptionForMan", uipdman.toString());
    }
  }

  private void addParamUsageInformationToContext(VelocityContext wrapperContext) {
    // user input parameters
    StringBuilder uip = new StringBuilder("");
    ...
    for (Parameter param : operation.getInputs().getParameter()) {
      ...
      for (ParameterPossibleValue possibleValue : param.getPossibleValue()) {
        uipd.append((uipd.length() != 0 ? "\n\t\t" : "") + "\\\"" + possibleValue.getValue() + "\\\" > "
          + possibleValue.getDescription());
        uipdman.append("\t\"" + possibleValue.getValue() + "\" > " + possibleValue.getDescription() + "\n\n");
      }

    }
    wrapperContext.put("usageParamParameter", uip.toString());
    wrapperContext.put("usageParamParameterDescription", uipd.toString());
    wrapperContext.put("usageParamParameterDescriptionForMan", uipdman.toString());
  }

  private void addOutputUsageInformationToContext(VelocityContext wrapperContext) {
    if (operation.getOutputs().getStdout() != null) {
      wrapperContext.put("usageOutputParameter", "-o STDOUT");
      wrapperContext.put("usageOutputParameterDescription", "-o STDOUT > Write output to the STDOUT");
    } else {
      // user input parameters
      StringBuilder uip = new StringBuilder("");
      // user input parameters description to print on the bash usage
      ...
      for (Output output : operation.getOutputs().getOutput()) {
        String value = "-o " + output.getName() + (i == 0 ? "|STDOUT" : "");
        uip.append((uip.length() == 0 ? "" : " "));
        ...
        String outputDescription = value + " > " + output.getDescription()
      ...
      wrapperContext.put("usageOutputParameterDescription", uipd.toString());
      wrapperContext.put("usageOutputParameterDescriptionForMan", uipdman.toString());
    }
  }

  private boolean generateDebianPackage(VelocityContext context) {
    return setupEnvironment(context) && copyExternalFiles() && buildPackage() && copyPackage();
  }
  ...
  private boolean installExtraFilesIfAny(VelocityContext context) {
    ...
    File directoryWithFilesToBeInstalled = new File(inputDir, Constants.BASHGENERATOR_INSTALL_OUTDIRNAME);
    List<String> installList = new ArrayList<String>();

    if (directoryWithFilesToBeInstalled.exists() && directoryWithFilesToBeInstalled.isDirectory()) {
      ...
      try {
        ...
        // being built
        FileUtils.copyDirectory(directoryWithFilesToBeInstalled, tempDebianDir);
        ...
        log.error("Error copying \"" + directoryWithFilesToBeInstalled + "\" to \"" + tempDebianDir + "\"");
      }
  ...
  private void verifyExecutionPermissionsOfFilesBeingInstalled(File file) {
    ...
    String relativePath = absolutePath.replaceFirst(
      new File(inputDir, Constants.BASHGENERATOR_INSTALL_OUTDIRNAME).getAbsolutePath(), "");
    if (file.isFile()) {
  ...
  private boolean loadDebianTemplates() {
    ...
    if (debianTemplates == null) {
      ...
      for (String file : DEBIAN_FILES) {
        debianTemplates.put(file, loadVelocityTemplateFromResources(DEBIAN_TEMPLATE_DIRECTORY_NAME + file));
      }
    }
    return success;
  }
  ...
  private String processInstallationInformation(Installation installation) {
    ...
    if (installation != null) {
      List<OperatingSystemDependency> operatingSystemList = installation.getOperatingSystem();

      for (OperatingSystemDependency osd : operatingSystemList) {
        if ("Ubuntu".equalsIgnoreCase(osd.getOperatingSystemName().toString())
          || "Debian".equalsIgnoreCase(osd.getOperatingSystemName().toString())) {
          List<PackageManager> packageManagerList = osd.getPackageManager();
          for (PackageManager packageManager : packageManagerList) {
            if ("Dpkg".equalsIgnoreCase(packageManager.getType().name())) {
              sb.append((i == 0 ? "" : ",") + packageManager.getConfig().toString());
              i++;
            }
          }
          break;
  ...
  private boolean addInformationToDebianTemplates(VelocityContext context) {
    SimpleDateFormat sdf = new SimpleDateFormat(RFC_822_DATE_PATTERN, Locale.ENGLISH);
    ...
    context.put("toolDependencies", processInstallationInformation(installation));
    ...
    for (Entry<String, Template> debianTemplateEntry : debianTemplates.entrySet()) {
      ...
      if (aggregate && debianTemplateEntry.getKey().startsWith("MAN.pod")) {
        for (Operation op : tool.getOperations().getOperation()) {
          ...
          Utils.writeTemplateContent(tempDebianInnerDir, null,
            debianTemplateEntry.getKey().replaceFirst("MAN.pod", op.getName() + ".pod")
              .replaceFirst("MAN", wrapperName), sw, false);
        }
        setOperation(null);
      } else {
        debianTemplateEntry.getValue().merge(context, sw);
        Utils.writeTemplateContent(tempDebianInnerDir, null,
          debianTemplateEntry.getKey().replaceFirst("MAN", wrapperName), sw, false);
      }
    }
  ...
  private boolean copyExternalFiles() {
    ...
    if (aggregate) {
      for (Operation op : tool.getOperations().getOperation()) {
        ...
        success = success
          && Utils.copyFile(new File(inputDir, Constants.BASHGENERATOR_WRAPPER_OUTDIRNAME + "/" + filename), new File(
            tempDebianDir, filename), true);

        // copy the workflow to the temporary directory
        filename = op.getName() + Constants.BASHGENERATOR_WORKFLOW_EXTENSION;
        success = success
          && Utils.copyFile(new File(inputDir, Constants.BASHGENERATOR_WORKFLOW_OUTDIRNAME + "/" + filename), new File(
      ...
      // copy the workflow to the temporary directory
      filename = operation.getName() + Constants.BASHGENERATOR_WORKFLOW_EXTENSION;
      success = success
        && Utils.copyFile(new File(inputDir, Constants.BASHGENERATOR_WORKFLOW_OUTDIRNAME + "/" + filename), new File(
          tempDebianDir, filename), false);
    }
  ...
  private boolean copyPackage() {
    ...
    for (File file : tempDebianBaseDir.listFiles(new DebianFileFilter())) {
      try {
        FileUtils.copyFileToDirectory(file, new File(outputDirectory, Constants.DEBIANBASHGENERATOR_DEBS_OUTDIRNAME));
        break;
      } catch (IOException e) {
  ...
  private void cleanEnvironment() {
    ...
    if (!deleteRes) {
      log.error("Error while deleting temporary directory \"" + tempDebianBaseDir + "\"");
    }
  }

  private static boolean areNonRequiredArgumentsThatCanBeCombinedInvalid(CommandLine cmd) {
    boolean res = false;

    // validate command-line parameters that are not required but need to be
    // used

    if ((!cmd.hasOption("d") || !cmd.hasOption(CMD_OPTIONS_DESCRIPTION_SHORT)) && cmd.hasOption("a")) {
      res = true;
    } else if ((!cmd.hasOption("a") || !cmd.hasOption(CMD_OPTIONS_DESCRIPTION_SHORT)) && cmd.hasOption("d")) {
      res = true;
    } else if ((!cmd.hasOption("a") || !cmd.hasOption("d")) && cmd.hasOption(CMD_OPTIONS_DESCRIPTION_SHORT)) {
      res = true;
    }
    return res;
  }

  private static int createOneDebianPackage(DebianBashWrapperGenerator dbwg, CommandLine cmd, Tool tool, File outputdir) {
    int exitCode = 0;
  ...
  private static int createSeveralDebianPackages(DebianBashWrapperGenerator dbwg, Tool tool, File outputdir) {
    int exitCode = 0;
    for (Operation operation : tool.getOperations().getOperation()) {
      dbwg.setWrapperName(operation.getName());

      boolean generationOK = dbwg.generateWrapper(tool, operation, outputdir);
      if (!generationOK) {
        log.error("Error generating Debian package for operation \"" + operation.getName() + "\"");
        exitCode = 4;
      }
    }
    return exitCode;
